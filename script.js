...
async function fetchAndRenderChart() {
  ...
  try {
    ...
    // ★ 日付ごとに秒数を合計する（例: "8/21": 900秒）
    const dailyTotals = {};
    data.forEach(item => {
      const d = new Date(item.created_at);
      const dateKey = `${d.getMonth() + 1}/${d.getDate()}`; // "8/21" のような形式
      dailyTotals[dateKey] = (dailyTotals[dateKey] || 0) + (item.seconds || 0);
    });

    const dailyEntries = Object.entries(dailyTotals); // [["8/21", 900], ["8/22", 1800]]
    const maxDailySeconds = Math.max(...dailyEntries.map(([_, sec]) => sec), 60);

    chartBox.innerHTML = dailyEntries.map(([dateStr, totalSec]) => {
      const min = Math.round(totalSec / 60 * 10) / 10;
      const heightPercent = Math.max(10, Math.min(100, (totalSec / maxDailySeconds) * 100));

      return `
        <div style="display:flex; flex-direction:column; align-items:center; flex:1; min-width:36px; height:100%; justify-content:flex-end;">
          <span style="font-size:10px; color:#888; margin-bottom:2px;">${min}分</span>
          <div style="width:100%; max-width:28px; height:${heightPercent}%; background:linear-gradient(180deg, #f0a2b8, #d87093); border-radius:4px 4px 0 0;"></div>
          <span style="font-size:10px; color:#666; margin-top:4px; font-weight:bold;">${dateStr}</span>
        </div>
      `;
    }).join('');
