...
async function saveToSupabase(seconds) {
  ...
  if (minutes < 1) return; // 1分未満は保存しない

  try {
    const { error } = await supabaseClient
      .from('work_sessions')
      .insert([{ duration: minutes }]);
    ...
    workChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: '作業時間 (分)',
          data: durations,
          backgroundColor: 'rgba(216, 112, 147, 0.6)', // タイマーの可愛い雰囲気に合わせたピンク
