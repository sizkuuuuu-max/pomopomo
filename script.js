...
let workChart = null; // グラフのインスタンス保持用

// Supabaseに作業時間を送信する関数
async function saveToSupabase(seconds) {
  if (!supabaseClient) return;
  if (seconds < 1) return; // 1秒未満は保存しない

  try {
    const { error } = await supabaseClient
      .from('work_sessions')
      .insert([{ seconds: seconds }]); // ★ seconds列に秒数を送信
...
// Supabaseからデータを取得してグラフを描画する関数
async function fetchAndRenderChart() {
  if (!supabaseClient || typeof Chart === "undefined") return;

  // キャンバス要素の安全チェック（nullエラー防止）
  const canvas = document.getElementById('workChart');
  if (!canvas) return; // 見つからなければ処理を抜ける

  try {
    // データ取得（作成日時順）
    const { data, error } = await supabaseClient
      .from('work_sessions')
      .select('seconds, created_at') // ★ seconds列を取得
    ...
    // グラフ用のラベル（時間）とデータ（分換算）を作成
    const labels = data.map(item => {
      const d = new Date(item.created_at);
      return `${d.getMonth() + 1}/${d.getDate()} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
    });

    // 保存されている秒数を分に換算してグラフに表示
    const minutesData = data.map(item => Math.round((item.seconds || 0) / 60 * 10) / 10);

    const ctx = canvas.getContext('2d');

    // 既にグラフが存在する場合は破棄して描き直し
    if (workChart) {
      workChart.destroy();
    }
    ...
    workChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: '作業時間 (分)',
          data: minutesData,
          backgroundColor: 'rgba(216, 112, 147, 0.6)', // ピンク色
...
// 画面の読み込み（HTML要素の配置）が完全に終わってからグラフを描画する
